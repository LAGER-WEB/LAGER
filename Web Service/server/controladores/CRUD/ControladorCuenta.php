...
class ControladorCuenta extends ControladorBase
{
   ...
   {
      $sql = "INSERT INTO Cuenta (idCuenta,idTipoUsuario,nombre,clave) VALUES (?,?,?,?);";
      $parametros = array($cuenta->idCuenta,$cuenta->idTipoUsuario,$cuenta->nombre,$cuenta->clave);
   ...
   {
      $parametros = array($cuenta->idCuenta,$cuenta->idTipoUsuario,$cuenta->nombre,$cuenta->clave,$cuenta->id);
      $sql = "UPDATE Cuenta SET idCuenta = ?,idTipoUsuario = ?,nombre = ?,clave = ? WHERE id = ?;";
      $respuesta = $this->conexion->ejecutarConsulta($sql,$parametros);
      if(is_null($respuesta[0])){
         return true;
      }else{
